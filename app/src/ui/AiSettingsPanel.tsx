...
export default function AiSettingsPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-y-auto px-4 pb-4">
      ...
      <p className="mt-1 rounded-lg bg-slate-100 p-3 text-xs text-slate-600">
        {disclosureFor(settings)}
      </p>
        ...
        {MODES.map((m) => (
          <button
            ...
            className={`rounded-md px-2 py-2 text-sm font-medium ${
              settings.mode === m.id ? 'bg-sky-500 text-white' : 'bg-white text-slate-600 shadow-sm'
            }`}
          >
            {m.label}
          </button>
      ...
      {settings.mode && (
        <div className="mt-4 flex gap-2">
          <button
            type="button"
            className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-600"
            disabled={validating}
