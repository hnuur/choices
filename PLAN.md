...
- [ ] **Phase 13 — Web lookup**: Ask AI and ramble can look up
      ...
      **provider-native search**, not an app-side search API (CORS plus
      ...
      places (restaurants, shops, venues) as options **or in
      prose**, use this exact shape per place: name, then a
      one-sentence why-it’s-good blurb — never paste search-result
      cards. Forbidden on place lines (and in place notes): street
      address, neighborhood, city, postal/ZIP, phone, website/URL
      (incl. Maps links), star ratings, review counts, open/closed,
      price ranges, and markdown. For place recommendations, do
      not cite per-place websites; other factual citations may
      still appear once at the end by publication name only (no
      place URLs). Proposals still go through the existing
      approval card (`setScore` / `createDecision` — no new
      payload type). Citations live in the assistant prose, not a
      new card row.
...
| Place option blurbs | user 2026-08-16: place recommendations (prose or options) are Name + one-sentence why-it’s-good only — never search-result cards with address/postal/phone/website/Maps/stars/hours/price/markdown. Per-place website citations forbidden (they conflicted with “cite by URL” and kept dumping OpenAI place cards); other facts may cite publication name only at the end. |
